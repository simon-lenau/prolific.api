...
list_of_prescreeners[grepl("custom", tolower(cls)), ]
list_of_prescreeners[, cls_type := gsub("web\\.eligibility\\.models\\.(.*?)eligibilityrequirement", "\\1", tolower(cls))]
...
x <- list_of_prescreeners[!grepl("selectanswer", tolower(cls)), ][8, ]
x <- list_of_prescreeners[grepl("selectanswer", tolower(cls)), ][1, ]
x <- list_of_prescreeners[1, ]
# list_of_prescreeners[, .N, keyby = "cls_type"]

# cat(paste0("\"", list_of_prescreeners[, .N, keyby = "cls_type"]$cls_type, "\" = TRUE", collapse = ",\n"), "\n")
...
prescreener_details_fmt <-
    function(x, ncol = 4) {
        ...
        if (length(prescreener_examples) > 0 & (nrow * ncol) > 0) {
            examples_tbl <- matrix(c(prescreener_examples, rep("", nrow * ncol - length(prescreener_examples))), ncol = ncol, byrow = TRUE)
...
color_fun <- tableau_color_pal(palette = "Tableau 20")
